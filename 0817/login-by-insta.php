...
if(empty($_GET['code'])){
	...
	$stmt = $dbh->prepare("select * from user where instagram_id=:user_id limit 1");
	$stmt->execute(array(":user_id"=>$json->user->id));
	$user = $stmt->fetch();
	//もし新規ユーザなら格納
	if(empty($user)){
		$stmt = $dbh->prepare("insert into user(instagram_id, instagram_access_token, created, modified) values
